...
def write_model_cfg(old_path, new_path, new_module_defs):
    ...
    for i, module_def in enumerate(new_module_defs):
        ...
        if mtype == "convolutional":
            ...
            """changed this to correctly parse anchor boxes"""
            lines.append("anchors = {}\n".format(", ".join([str(int(x)) for line in module_def['anchors'] for x in line])))
...
##  output a new cfg file
    ...
    # Run NMS
    print("INF OUT",inf_out)
    output = non_max_suppression(inf_out, conf_thres=0.005, iou_thres=0.5)
